...
step5_ssh_hardening() {
    ...
    echo ""
    echo "请输入您的SSH公钥（完整的公钥内容，例如: ssh-rsa AAAAB3...）："
    read -p "> " SSH_PUBLIC_KEY

    if [ -z "$SSH_PUBLIC_KEY" ]; then
        echo "警告：未输入SSH公钥，跳过公钥配置。"
        echo "您稍后需要手动添加公钥到 /root/.ssh/authorized_keys"
    else
...
step6_substore_config() {
    ...
    MANAGE_SCRIPT_URL="https://raw.githubusercontent.com/kakakakaka1/init/main/manage_substore_new.sh"
    MANAGE_SCRIPT_LOCAL_PATH="/root/manage_substore_new.sh"
    ...
    if ! grep -q "alias singbox=" /root/.bashrc 2>/dev/null; then
        echo "alias singbox='sing-box; bash /root/manage_substore_new.sh'" >> /root/.bashrc
        echo "别名已添加。"
    else
        echo "别名已存在，跳过添加。"
    fi
